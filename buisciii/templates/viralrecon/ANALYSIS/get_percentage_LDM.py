...
lineage_excel_pattern = (
    "/data/ucct/bi/references/outbreakinfo/*_mutaciones_definitorias_linaje.csv"
)
pwd_directory = "./"
...
def group_deletions(processed_data):
    ...
    for gene, positions in deletion_groups.items():
        ...
        for start, end in ranges:
            if start == end:
                formatted_deletions.append(
                    {"mutation": f"{gene}:del{start}/{start}", "type": "deletion"}
                )
            else:
                formatted_deletions.append(
                    {"mutation": f"{gene}:del{start}/{end}", "type": "deletion"}
                )
...
def process_and_verify(input_file, lineage_excel, output_tsv, sample_name):
                    ...
                    processed_data.append(
                        {
                            "lineage": lineage,
                            "type": mutation_type,
                            "mutation": normalize_string(mutation),
                        }
                    )
    ...
    for _, row in processed_df.iterrows():
        ...
        match = lineage_df[
            (lineage_df["mutation"] == mutation_csv)
        ...
        if not match.empty:
            ...
            exists = 0
            linages_list = (
                lineage_df[lineage_df["mutation"] == mutation_csv]["lineage"]
                .unique()
                .tolist()
            )
    ...
    for excel_file in excel_files:
        ...
        try:
            ...
            if "sample" in df.columns:
                ...
                results.append(
                    {"sample": sample_name, "% LDMutations": percentage_ones}
                )
            else:
